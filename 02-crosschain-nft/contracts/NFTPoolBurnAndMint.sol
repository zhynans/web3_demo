...
contract NFTPoolBurnAndMint is CCIPReceiver, OwnerIsCreator {
    ...
    function burnAndSendNFT(
        ...
        // transfer NFT to this adrdress to burn the NFT
        wnft.transferFrom(msg.sender, address(this), tokenId);

        // burn the wnft before send to ccip
        wnft.burn(tokenId);

        bytes memory playload = abi.encode(tokenId, newOwner);
        return sendMessagePayLINK(chainSelector, receiver, playload);
    }
